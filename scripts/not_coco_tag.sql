...
BEGIN
    ...
    account_tokens AS (
        SELECT aid.ACCOUNT_NAME_UPPER, SUM(f.TOTAL_TOKENS) AS Q2_TOKENS
        FROM SNOWSCIENCE.LLM.CORTEX_CODE_USER_DAY_FACT f
        INNER JOIN account_ids aid ON f.ACCOUNT_ID = aid.ACCOUNT_ID
        WHERE f.DS >= :Q_START
        GROUP BY aid.ACCOUNT_NAME_UPPER
    ...
    SELECT
        ...
        -- #notcoco always wins: if IS_NOT_COCO=TRUE, IS_COCO_FINAL=FALSE regardless of tokens
        CASE WHEN IS_NOT_COCO = TRUE THEN FALSE
             WHEN (IS_COCO = TRUE
                   AND NOT (COCO_SOURCE = 'PARTNER_COMMENTS' AND HAS_ACCOUNT_COCO = FALSE AND Q2_TOKENS = 0))
                  OR (IS_COCO = FALSE AND HAS_ACCOUNT_COCO = TRUE)
             THEN TRUE
             ELSE FALSE
        END AS IS_COCO_FINAL
    FROM uc_joined;
